...
- 双击机器人头像开启生图功能，再次双击机器人头像关闭生图功能。
- 开启后发送 `提示词` 即可生图。
- 未开启时发送 `/生图 提示词` 可临时生图。
- `/批量生成 3 提示词` 批量文生图。
- 生图功能开启后，普通图片消息静默缓存到当前会话。
- 当前图、引用图、缓存图的参考图优先级；临时生图不使用缓存图。
- `/重置` 清空当前会话参考图缓存。
...
`/生图 提示词` 仅用于未开启生图功能时临时生图；临时图生图只使用当前消息图片或引用图片，不使用缓存图。
`/批量生成` 需要先开启生图功能，数量最大按 4 张执行。
